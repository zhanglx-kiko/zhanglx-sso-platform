 ...
 */
public interface IBaseMapperX<T> extends BaseMapper<T> {

    default IPage<T> selectPage(BasePageQuery pageParam, @Param("ew") Wrapper<T> queryWrapper) {
        // MyBatis Plus 查询
    ...
    default void updateBatch(Collection<T> entities) {
        entities.forEach(item -> update(item, new QueryWrapper<>()));
    }
